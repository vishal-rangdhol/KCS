...
function ProductCard({ product, index }: { product: typeof products[0], index: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Content Layer - Recalibrated for mobile density (30% reduced height feel) */}
      <div style={{ transform: "translateZ(30px)" }} className="relative z-20 p-4 sm:p-12 flex-1 flex flex-col justify-between">
        {/* Header Terminal */}
        <div className="mb-2 md:mb-8">
          <div className="flex items-center gap-2 mb-2">
            <span className="px-2 py-0.5 rounded-full bg-primary/10 border border-primary/20 text-primary text-[8px] md:text-[10px] font-bold uppercase tracking-widest font-headline">
              {product.tag}
            </span>
          </div>
          <h3 className="text-xl sm:text-5xl font-bold tracking-tighter text-foreground group-hover:text-primary transition-colors duration-500 font-headline leading-none mb-2 md:mb-6">
            {product.name}
          </h3>
        ...
        {/* Features Protocol - Tightened Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 md:gap-4 mb-3 md:mb-10">
          {product.features.map((feature, i) => (
            <div key={i} className="flex items-start gap-1.5 md:gap-3 p-1.5 md:p-4 rounded-lg md:rounded-2xl bg-white/5 border border-white/5 group-hover:border-white/10 transition-all">
              <CheckCircle2 className="w-3 h-3 md:w-5 md:h-5 text-primary shrink-0 mt-0.5" />
              <span className="text-[8px] md:text-sm font-medium text-foreground/80">{feature}</span>
            </div>
          ))}
        </div>

        {/* Vision Node - Compressed Footer */}
        <div className="pt-2 md:pt-8 border-t border-white/5 mb-3 md:mb-8">
          ...
          <p className="text-[10px] md:text-lg font-bold text-foreground leading-tight italic tracking-tight">
            "{product.vision}"
          </p>
        </div>
        ...
        <Button 
          variant="outline" 
          className="h-8 md:h-14 px-4 md:px-8 rounded-full border border-primary/20 text-primary bg-background/40 backdrop-blur-md hover:bg-primary hover:text-white hover:border-primary transition-all duration-500 text-[9px] md:text-sm font-bold group/btn shadow-xl w-full sm:w-fit"
          onClick={() => {
    ...
    <Chapter id="products" className="bg-background py-16 md:py-32 overflow-visible">
      <div className="text-center mb-10 md:mb-24 w-full px-4 md:px-6">
        ...
        >
          ...
          <motion.p 
            ...
            className="text-muted-foreground mt-4 md:mt-10 w-full max-w-3xl mx-auto text-sm md:text-2xl leading-relaxed italic font-medium"
          >
      ...
      <div className="w-full relative px-4 md:px-12">
        ...
        >
          ...
          <div className="flex justify-center md:justify-end gap-3 md:gap-6 mt-8 md:mt-24">
